...
async function addTrelloTasksToBoard(name, description, dueDate, start, labels) {
    try {
        if (!apiKey || !token || !toDoListId) {
            return {
                ...
                data: {
                    message: "One or more required env vars are missing: TRELLO_API_KEY, TRELLO_TOKEN, TRELLO_LIST_ID",
                },
            };
        }
...
server.tool("add-trello-tasks", "Add a new card to a specified list on a specific board", {
    ...
    try {
        const apiResponse = await addTrelloTasksToBoard(name, description, dueDate, start, labels);
